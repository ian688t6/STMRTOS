...
//size:要分配的内存大小(字节)
//返回值:新分配到的内存首地址.
void *rtos_mem_realloc(void *ptr,u32 size)  
{  
    u32 offset;  
    offset=mem_malloc(size);  
    if(offset==0XFFFFFFFF)return NULL;     
    ...
    {  									   
	    rtos_memcpy((void*)((u32)mallco_dev.membase+offset),ptr,size);	//拷贝旧内存内容到新内存   
        rtos_mem_free(ptr);  											  	//释放旧内存
...
//*src:源地址
//n:需要复制的内存长度(字节为单位)
void rtos_memcpy(void *des,void *src,u32 n)  
{  
    u8 *xdes=des;
	u8 *xsrc=src; 
    while(n--)*xdes++=*xsrc++;  
...
void rtos_memset(void *s,u8 c,u32 count)  
{  
    u8 *xs = s;  
    while(count--)*xs++=c;  
}
...
{  
    rtos_memset(mallco_dev.memmap, 0,memtblsize*2);//内存状态表数据清零  
	rtos_memset(mallco_dev.membase, 0,memsize);	//内存池所有数据清零  
